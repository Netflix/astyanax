...
class MessageConsumerImpl implements MessageConsumer {
    ...
    List<MessageContext> readMessagesFromShardUsingDefaultLock(String shardName, int itemsToPop) throws MessageQueueException, BusyLockException {
        ...
        try {
            ...
            lockColumn = MessageQueueEntry.newLockEntry(shardName, MessageQueueEntryState.None);
...
//                                .putColumn(history.getToken(), queue.serializeToString(context.getHistory()), queue.queueInfo.getHistoryTtl()); // TTL
//                    } catch (Exception e) {
...
//                                            .putColumn(timeoutEntry, column.getStringValue(), queue.queueInfo.getRetentionTimeout());
...
//                                                .putEmptyColumn(messageIdEntry, queue.queueInfo.getRetentionTimeout());
//                                    }
