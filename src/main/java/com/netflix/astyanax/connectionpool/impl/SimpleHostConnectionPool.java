...
public class SimpleHostConnectionPool<CL> implements HostConnectionPool<CL> {
	...
	/**
	 * Interface to notify the owning connection pool of up/down state changes.  This
	 * give the owning connection pool a chance to remove a downed host from its internal
	 * state until it's back up.
	 * @author elandau
	 *
	 * @param <CL>
	 */
	...
	private Connection<CL> waitForConnection(int timeout) throws ConnectionException {
		...
		try {
			if (blockedThreads.incrementAndGet() <= config.getMaxBlockedThreadsPerHost()) {
				...
				throw new PoolTimeoutException("Too many clients blocked on this pool " + blockedThreads.get()).setHost(getHost());
			}				
	...
	public boolean returnConnection(Connection<CL> connection) {
		...
		if (activeCount.get() <= config.getMaxConnsPerHost()) {
			...
			// maxConnsPerHost was reduced.  This may end up closing too many connections,
				...
				executor.schedule(new Runnable() {
					...
					public void run() {
						...
						if (reconnect()) {
							...
							// Created a new connection successfully.  Update internal state.
	...
	/**
	 * Try to open a new connection asynchronously.  We don't actually return a connection here.
	 * Instead, the connection will be added to idle queue when it's ready.
	 */
					...
					try {
						...
						connection.openAsync(new Connection.AsyncOpenCallback<CL>() {
			                ...
			                public void success(Connection<CL> connection) {
			                    ...
			                    // Sanity check in case the connection pool was closed
			                    if (isShutdown()) {
			                    	discardIdleConnections();
			                    }
			                }
