...
public class TokenPartitionedTopology<CL> implements Topology<CL> {
	...
	/**
	 * Comparator used to find the connection pool to the host which owns a specific token
	 */
	...
	public synchronized boolean setPools(Map<BigInteger, Collection<HostConnectionPool<CL>>> ring) {
		...
		if (didChange) {
			List<HostConnectionPoolPartition<CL>> partitions = new
				ArrayList<HostConnectionPoolPartition<CL>>(tokens.values());
