...
/**
 * Utility to cache mappers. There's a small performance hit to reflect on
 * a bean. This cache, re-uses mappers for a given bean
 */
public class MappingCache {
     ...
     *
     * @param clazz class for the mapper
     * @return mapper
     */
     ...
     *
     * @param clazz class for the mapper
     * @param annotationSet annotation set for the mapper
     * @return mapper
     */
    @SuppressWarnings({"unchecked"})
    public<T> Mapping<T> getMapping(Class<T> clazz, AnnotationSet<?, ?> annotationSet) {
        Mapping<T> mapping = (Mapping<T>)cache.get(clazz);    // cast is safe as this instance is the one adding to the map
        if ( mapping == null ) {
