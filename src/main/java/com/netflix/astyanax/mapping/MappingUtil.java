...
/**
 * Higher level mapping functions. Methods that behave similar
 * to a Map.
 */
public class MappingUtil {
    ...
    /**
     * @param keyspace keyspace to use
     * @param annotationSet annotation set to use
     */
     ...
     *
     * @param columnFamily column family of the item
    ...
    public <T, K> void remove(ColumnFamily<K, String> columnFamily, T item) throws Exception {
        ...
        @SuppressWarnings({"unchecked"})
        Class<K> idFieldClass = (Class<K>)mapping.getIdFieldClass();    // safe - after erasure, this is all just Class anyway
    ...
    public <T, K> List<T> getAll(ColumnFamily<K, String> columnFamily, Class<T> itemClass) throws Exception {
        Mapping<T> mapping = getMapping(itemClass);
        Rows<K, String> result = keyspace.prepareQuery(columnFamily).getAllRows().execute().getResult();
    ...
    public <T> Mapping<T> getMapping(Class<T> clazz) {
        return (cache != null) ? cache.getMapping(clazz, annotationSet) : new Mapping<T>(clazz, annotationSet);
    }
}
