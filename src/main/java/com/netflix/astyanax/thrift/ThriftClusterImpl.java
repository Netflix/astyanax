...
public class ThriftClusterImpl implements Cluster {
	...
	public String describeSnitch() throws ConnectionException {
		return connectionPool.executeWithFailover(
				new AbstractOperationImpl<String>(
						tracerFactory.newTracer(CassandraOperationType.DESCRIBE_SNITCH)) {
			@Override
			public String internalExecute(Client client) throws Exception {
	...
	public String dropColumnFamily(final String keyspaceName, final String columnFamilyName) throws OperationException, ConnectionException {
		return connectionPool.executeWithFailover(
				new AbstractOperationImpl<String>(
						tracerFactory.newTracer(CassandraOperationType.DROP_COLUMN_FAMILY)) {
			@Override
			public String internalExecute(Client client) throws Exception {
	...
	public String updateKeyspace(final KeyspaceDefinition def) throws ConnectionException {
		return connectionPool.executeWithFailover(
			...
			@Override
			public String internalExecute(Client client) throws Exception {
				return client.system_update_keyspace(((ThriftKeyspaceDefinitionImpl)def).getThriftKeyspaceDefinition());
			}
		}, config.getRetryPolicy().duplicate()).getResult();
	}
