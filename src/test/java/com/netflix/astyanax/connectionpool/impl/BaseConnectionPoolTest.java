...
public abstract class BaseConnectionPoolTest {
    ...
    public void testAll()  {
    	...
    	for (int i = 0; i < 5; i++) {
    		pool.addHost(new Host("127.0." + i + ".0", TestHostType.GOOD_FAST.ordinal()), true);
    	...
    	for (int i = 0; i < 10; i++) {
			try {
				OperationResult<String> result = pool.executeWithFailover(dummyOperation, RunOnce.get());
    ...
    public void testRollingRestart()  {
    	...
    	for (int i = 0; i < 5; i++) {
    		Host host = new Host("127.0." + i + ".0", TestHostType.GOOD_FAST.ordinal());
    ...
    public void testAlwaysDown()  {
    	...
    	pool.addHost(new Host("127.0.0.1", TestHostType.ALWAYS_DOWN.ordinal()), true);
    ...
    public void testConnectTimeout()  {
    	...
    	pool.addHost(new Host("127.0.0.1", TestHostType.CONNECT_TIMEOUT.ordinal()), true);
    ...
    public void testTimeoutTimeout()  {
    	...
    	pool.addHost(new Host("127.0.0.1", TestHostType.SOCKET_TIMEOUT.ordinal()), true);
    ...
    public void testConnectBadRequest()  {
    	...
    	pool.addHost(new Host("127.0.0.1", TestHostType.CONNECT_BAD_REQUEST_EXCEPTION.ordinal()), true);
    ...
    public void testGoodFast()  {
    	...
    	pool.addHost(new Host("127.0.0.1", TestHostType.GOOD_SLOW.ordinal()), true);
    ...
    public void testDefaultConfig() {
    	...
    	try {
	    	ConnectionPool<TestClient> pool = 
	    		new RoundRobinConnectionPoolImpl<TestClient>(config, 
	...
	public void testAddHostThatIsDown() {
    	...
    	config.setRetryBackoffStrategy(new FixedRetryBackoffStrategy(100, 0));
    	...
    	Host host1 = new Host("127.0.0.1", TestHostType.CONNECT_FAIL_FIRST.ordinal());
    	Map<BigInteger, List<Host>> ring1 = Maps.newHashMap();
    	ring1.put(new BigInteger("0"), Lists.newArrayList(host1));
	...
	public void testConnectionAborted() {
    	...
    	Host host = new Host("127.0.0.1", TestHostType.ABORTED_CONNECTION.ordinal());
    	pool.addHost(host, true);
