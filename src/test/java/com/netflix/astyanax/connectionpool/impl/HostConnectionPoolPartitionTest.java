...
public class HostConnectionPoolPartitionTest {
	...
	public void testPartition() {
		...
		List<TestHostConnectionPool> pools = Arrays.asList(makePool(1), makePool(2), makePool(3), makePool(4));
		
		List<TestHostConnectionPool> pool1 = Arrays.asList(pools.get(0), pools.get(1), pools.get(2));
		List<TestHostConnectionPool> pool2 = Arrays.asList(pools.get(0), pools.get(3));
	...
	public void testTopology() {
		...
		TokenPartitionedTopology<TestClient> topology = new TokenPartitionedTopology<TestClient>(strategy);
		...
		Map<BigInteger, Collection<HostConnectionPool<TestClient>>> flatRing = Maps.newHashMap();
		...
		HostConnectionPoolPartition<TestClient> partition = topology.getPartition(null);
		Assert.assertEquals(nHosts, partition.getPools().size());
		
		for (int i = 0; i < nHosts; i++) {
			partition = topology.getPartition(new BigInteger(Integer.toString(i * 1000)));
			Assert.assertEquals(new BigInteger(Integer.toString(i*1000)), partition.id());
		}
		
		for (int i = 0; i < nHosts; i++) {
			partition = topology.getPartition(new BigInteger(Integer.toString(i * 1000 + 500)));
			Assert.assertEquals(new BigInteger(Integer.toString(i*1000)), partition.id());
		}
		
		Map<BigInteger, Collection<HostConnectionPool<TestClient>>> emptyRing = Maps.newHashMap();
