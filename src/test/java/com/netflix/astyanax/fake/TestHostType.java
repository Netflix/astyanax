...
public enum TestHostType {
	ALWAYS_DOWN {
		@Override
		public <R> OperationResult<R> execute(HostConnectionPool<TestClient> pool, Operation<TestClient, R> op)
	...
	GOOD_SLOW {
		...
		public <R> OperationResult<R> execute(HostConnectionPool<TestClient> pool, Operation<TestClient, R> op)
				throws ConnectionException {
			return new OperationResultImpl<R>(pool.getHost(), op.execute(null), think(500));
		}
	...
	GOOD_FAST {
		...
		public <R> OperationResult<R> execute(
				...
				Operation<TestClient, R> op) throws ConnectionException {
			return new OperationResultImpl<R>(pool.getHost(), op.execute(null), think(5));
		}
	...
	private static final Map<Integer,TestHostType> lookup 
    	= new HashMap<Integer,TestHostType>();
